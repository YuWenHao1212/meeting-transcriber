...
class TestTranscribeCommand:
  ...
  def test_transcribe_custom_output(self, mock_fmt, mock_transcribe, tmp_path):
    ...
    mock_transcribe.return_value = TranscriptResult(
      full_text="test", duration=5.0, cost=0.0, engine="openai",
    )
    mock_fmt.return_value = "# Transcript"

    result = runner.invoke(app, [
      "transcribe", str(audio), "--output", str(out),
    ])
...
class TestSummarizeCommand:
  ...
  def test_summarize_with_playbook(self, mock_summarize, tmp_path):
    ...
    result = runner.invoke(app, [
      "summarize", str(transcript), "--playbook", str(playbook),
    ])
...
class TestInitCommand:
  ...
  def test_init_creates_config(self, mock_init):
    mock_init.return_value = Path("/tmp/test-config")
